...
public class MyFogDevice extends PowerDatacenter {
	...
	protected void updateTimingsOnSending(Tuple resTuple) {
		...
		// WILL NEED TO CHECK IF A NEW LOOP STARTS AND INSERT A UNIQUE TUPLE ID
		// TO IT.
	...
	protected void sendTupleToMyActuator(Tuple tuple) {
		 ...
		 * actuatorAssociation.getSecond(); if(actuatorId ==
		 * tuple.getMyActuatorId()){ send(actuatorId, delay,
		 * FogEvents.TUPLE_ARRIVAL, tuple); return; } } int childId =
		 * getChildIdForTuple(tuple); if(childId != -1) sendDown(tuple,
		 * childId);
		 */

...
import org.fog.placement.MyController_A;
import org.fog.placement.MyModulePlacement;
import org.fog.placement.MyModulePlacementBF;
...
public class Application_A {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// Aqui começa o mapeamento dos módulos para execução da aplicação
			
			// Primeiro os módulos de armazenamento é alocado na nuvem.
			moduleMapping.addModuleToDevice("storageModule", "cloud"); 
			
			// Em seguida, cada end device recebe o módulo clientModule			
			...
			MyController_A controller = new MyController_A("master-controller", fogDevices, sensors, actuators);
			
			//////////////////////////////////////
			//Estratégias de placement
			
			// MyModulePlacement
			
			// Aqui o módulo principal é enviado para o ambiente de execução para ser distribuído entre os fog nodes
			...
			// Aqui o módulo principal é enviado para o ambiente de execução para ser distribuído entre os fog nodes
			controller.submitApplication(application, 0, new MyModulePlacementBF(fogDevices, sensors, actuators, application, moduleMapping,"mainModule"));
			//controller.submitApplication(application, 0, new MyModulePlacementBF(fogDevices, sensors, actuators, application, moduleMapping,"mainModule"));
	...
	private static MyApplication createApplication(String appId, int userId){
		...
		application.addAppModule("storageModule2", 10, 50, 12000, 100);
		...
		application.addAppEdge("mainModule", "storageModule", 1000, 300, "StoreData", Tuple.UP, AppEdge.MODULE);
		application.addAppEdge("mainModule", "storageModule", 1000, 300, "StoreData", Tuple.DOWN, AppEdge.MODULE);
		...
		// Aqui é criado um deadline aleatório para o módulo principal, isso faz com que a necessidade de deadline de 
		// cada módulo seja alterada a cada execução.

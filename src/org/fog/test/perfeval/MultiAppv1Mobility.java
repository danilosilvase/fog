...
public class MultiAppv1Mobility {
	...
	static String mobilityDestination = "e-0-0";
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < idOfEndDevices.size(); i++) {
				...
				// moduleMapping.addModuleToDevice("moduleA_1",
				// fogDevice.getName());
			}

			
				
				for (int j = 0; j < idOfEndDevices.size(); j++) {
					MyFogDevice fogDevice = deviceById.get(idOfEndDevices.get(j));
					Pair<Double, Integer> pair = new Pair<Double, Integer>(0.00, idOfEndDevices.get(j));
					mobilityMap.put(fogDevice.getId(), pair);
				}
			
			MyController controller = new MyController("master-controller", fogDevices, sensors, actuators);

			/*
			 * Em ordem hierárquica, cada dispositivo de Nevoeiro de nível
			 * particular é conectado com o nível superior. Aqui, consideramos a
			 * mobilidade de dispositivos de Nevoeiro arbitrários de nível
			 * inferior para um determinado destino.
			 */

			/*
			 * No Snippet de código, os usuários podem adicionar outras
			 * instruções necessárias ao método manageMobility para lidar com
			 * problemas relacionados à mobilidade, como a migração do AppModule
			 * e a conexão com a latência.
			 */
			...
			controller.submitApplication(application, 0, (CLOUD)
					...
					: (new MyModulePlacement(fogDevices, sensors, actuators, application, moduleMapping, "moduleB_1")));
	...
	private static MyFogDevice addEnd(String endPartialName, int userId, String appId, int parentId) {
		...
		// Mobility code
		getIdByName.put(end.getName(), end.getId());

		/*
		 * if ((int) (Math.random() * 100) % 2 == 0) { Pair<Double, Integer>
		 * pair = new Pair<Double, Integer>(0.00,
		 * getIdByName.get(mobilityDestination)); mobilityMap.put(end.getId(),
		 * pair);
		 * 
		 * }else{ Pair<Double, Integer> pair = new Pair<Double, Integer>(0.00,
		 * getIdByName.get(mobilityDestination2)); mobilityMap.put(end.getId(),
		 * pair);
		 * 
		 * }
		 */
		// end.setxCoordinate(getValue(10.00));
		// end.setyCoordinate(getValue(15.00));

		end.setxCoordinate((10.00));
		end.setyCoordinate((15.00));

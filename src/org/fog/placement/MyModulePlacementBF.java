...
public class MyModulePlacementBF extends MyPlacement{
	...
	public MyModulePlacementBF(List<MyFogDevice> fogDevices, List<MySensor> sensors, List<MyActuator> actuators, 
	...
	protected void mapModules() {
		...
		// Esse laço percorre todos os nós fogs e módulos, verifica se já existe mapeamento para o módulo 
			...
			if(device.getLevel()==1)
			{
				...
				for(int i = 0; i<keys.size()-1; i++)
				{
					for(int j=0;j<keys.size()-i-1;j++)
					{
						...
						// Verifica o deadline do nó atual com o proximo nó de e os ordena. 
				...
				//	Variável baseMipsOfPlacingModule recebe a necessidade em mips para um determinado módulo de aplicação 			
				...
				for(int key:keys)
				{
					...
					//Recebe o módulo e adiciona à variável additionalMips a necessidade de cada módulo
					...
					if(currentMips+baseMipsOfPlacingModule+additionalMips<device.getMips())
					{
						...
						//a key k such that (key==null ? k==null : key.equals(k)). (There can be at most one such mapping.)
